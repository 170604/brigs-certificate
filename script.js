...
function initCertificateApp() {
    ...
    const GAS_WEBHOOK_URL = "https://script.google.com/macros/s/AKfycbw8M-UW7Xxbbh2JAsLT0EDu_eGNF7aV9pjfs0vNrz0BT-3yyyMAITiLKZ7RAIKBKHMN/exec"; // PASTE YOUR URL HERE

    async function submitToGoogleSheet() {
        if (GAS_WEBHOOK_URL === "https://script.google.com/macros/s/AKfycbw8M-UW7Xxbbh2JAsLT0EDu_eGNF7aV9pjfs0vNrz0BT-3yyyMAITiLKZ7RAIKBKHMN/exec") return;
        const payload = {
            app: "certificate",
    ...
    function downloadAs(type) {
        ...
        const downloadBtn = type === 'pdf' ? downloadPdfBtn : downloadImgBtn;
        ...
        html2canvas(cardElement, { useCORS: true, scale: 3 }).then(canvas => {
            ...
            if (type === 'image') {
                ...
                pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, 0, 600, 400);
                pdf.save(`${employeeName}_Thank_You.pdf`);
            }
            if (type === 'image') link.click();
...
function initSalesQuizApp() {
    ...
    quizForm.addEventListener("submit", async (e) => {
        ...
        const empId = document.getElementById("salesEmployeeId").value.trim();
        ...
        questions.forEach((_, idx) => {
            ...
            if (selected) total += parseInt(selected.dataset.marks);
        });
        resultDiv.style.display = "block";
        resultDiv.innerHTML = `<h2>Score: ${total} / ${questions.length * 40}</h2>`;
        if (GAS_WEBHOOK_URL !== "https://script.google.com/macros/s/AKfycbw8M-UW7Xxbbh2JAsLT0EDu_eGNF7aV9pjfs0vNrz0BT-3yyyMAITiLKZ7RAIKBKHMN/exec") { // Use a placeholder for comparison") {
            try {
                await fetch(GAS_WEBHOOK_URL, { method: "POST", mode: "no-cors", body: JSON.stringify({ app: "salesQuiz", name, employeeId: empId, score: total }) });
                resultDiv.innerHTML += `<p>✅ Result saved.</p>`;
            } catch (err) { resultDiv.innerHTML += `<p>⚠️ Could not save result.</p>`; }
        }
    });
...
function initInterviewQuiz() {
    ...
    // --- Value and Culturally Based Questions ---
    ...
    function buildQuiz() {
        ...
        questions.forEach((qData, idx) => {
            ...
            for (const [key, val] of Object.entries(qData.options)) {
                const optLabel = document.createElement("label");
                optLabel.className = "option";
                optLabel.innerHTML = `<input type="radio" name="iq${idx}" value="${key}" /><span>${key}. ${val}</span>`;
                qDiv.appendChild(optLabel);
            }
            quizContainer.appendChild(qDiv);
        });
    ...
    quizForm.addEventListener("submit", async (e) => {
        ...
        const email = document.getElementById("interviewMailID").value.trim(); // Corrected ID
        const location = document.getElementById("location").value.trim(); // Corrected ID

        // --- VALIDATION LOGIC ---
        if (!name || !location || !email) { // Corrected validation check
            alert("Please fill in all your details: Name, Email, and Location.");
            return;
        }
        ...
        if (GAS_WEBHOOK_URL !== "https://script.google.com/macros/s/AKfycbw8M-UW7Xxbbh2JAsLT0EDu_eGNF7aV9pjfs0vNrz0BT-3yyyMAITiLKZ7RAIKBKHMN/exec") { // Use a placeholder for comparison
            try {
                // --- Updated payload to send correct data ---
                await fetch(GAS_WEBHOOK_URL, { 
                    ...
                    body: JSON.stringify({ 
                        ...
                        name, 
                        location, 
                        email, // Send email instead of empId
    ...
    document.getElementById("interviewQuizResetBtn").addEventListener("click", () => { quizForm.reset(); resultDiv.style.display = 'none'; });
    buildQuiz();
}